...
NewCompressFile = (path, key) => {
    ...
    let writeStream = fs.createWriteStream(path + '.cry');
    let buffer;
        ...
        if (buffer !== null) {
            buffer.forEach(integer => {
                ...
                binary_string.forEach(group_of_bits => {
                    ...
                    console.log(result);
                    writeStream.write(binary_to_int(result), 'binary');
                });
            });
        }
    });
